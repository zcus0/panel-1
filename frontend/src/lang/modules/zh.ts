...
const message = {
    ...
    aiTools: {
        agents: {
            ...
            discoverModelsFailedUseDefaults: '获取模型失败，已使用默认模型',
            ...
            validateImageAvailabilityHelper: '保存前会真实生成一张最小图片，可能产生费用',
        },
        model: {
            model: '模型',
            localModel: '本地模型',
    ...
    toolbox: {
        ...
        clam: {
            ...
            alertHelper: 'OSS 版支持定时扫描和短信告警功能',
    ...
    setting: {
        ...
        proxyHelper4: 'Docker 拉取镜像等网络请求将通过代理服务器访问',
        proxyHelper5: '系统类型脚本库的统一下载与同步',
        proxyHelper6: '申请证书',
        proxyType: '代理类型',
        proxyUrl: '代理地址',
        proxyPort: '代理端口',
    ...
    license: {
        ...
        licenseAlert: '仅当许可证正常绑定到节点时，该许可证才能添加节点，只有正常绑定到许可证的节点支持切换。',
        licenseUnbindHelper: '检测到该许可证存在节点，请解绑后重试！',
        subscription: '订阅',
        perpetual: '永久授权',
        product: '产品',
        ...
        authorizedUser: '被授权方',
        lostHelper: '许可证已达到最大重试次数，请手动点击同步按钮，以确保功能正常使用，详情：',
        exceptionalHelper: '许可证同步验证异常，请手动点击同步按钮，以确保功能正常使用，详情：',
        quickUpdate: '快速更新',
        unbindHelper: '解除绑定后将清除该节点所有相关设置，是否继续？',
        importLicense: '导入许可证',
        ...
        licenseRequiredUserTip: 'OSS 版许可证未绑定，请联系超级管理员导入许可证。',
        levelUpPro: 'OSS 版',
        licenseSync: '许可证同步',
        knowMorePro: '了解更多',
        ...
        serialNo: '序列号',
        remark: '备注',
        waf: '可以获得拦截地图、日志、封锁记录、地理位置封禁、自定义规则、自定义拦截页面等功能。',
        tamper: '可以保护网站免受未经授权的修改或篡改。',
        tamperHelper: '操作失败，该文件或文件夹已经开启防篡改，请检查后重试！',
        setting: '可以自定义面板 Logo、欢迎简介等信息。',
        monitor: '可以查看网站的实时状态、访客趋势、访客来源、请求日志等信息。',
        alert: '可通过短信接收告警信息，并查看告警日志，全面掌控各类关键事件，确保系统运行无忧。',
        node: '可以使用 1Panel 管理多台 linux 服务器。',
        nodeApp: '可以统一升级多节点应用版本，无需手动切换节点。',
        nodeDashboard: '可以集中管理多节点应用、网站、数据库和计划任务。',
        fileExchange: '可以在多台服务器之间快速传输文件。',
        app: '可通过手机 APP，查看服务信息、异常监控等。',
        cluster: '可以管理 MySQL/Postgres/Redis 主从集群。',
        vllm: '可将 vLLM 服务统一纳入 1Panel 管理，集中完成部署、配置调整、状态运维与任务追踪，降低本地模型服务维护成本。',
        'model-downloader':
            '可从 HuggingFace、ModelScope 或兼容镜像站搜索、下载并管理本地模型文件，统一维护本地模型目录与下载任务。',
    },
    clean: {
        scan: '开始扫描',
    ...
    xpack: {
        ldap: {
            ...
            importUsers: '导入 AD/LDAP 用户',
            searchUsersPlaceholder: '搜索用户名',
            readFromLDAP: '从 LDAP 读取',
            selectedUsers: '已选择 {0} 项',
            queriedAt: '读取时间',
            cachedResult: '缓存结果',
            roleForNewUsers: '新用户角色',
            selectRole: '请选择角色',
            roleOnlyForNewUsers: '仅在本次处理包含新用户时需要选择角色。',
            roleRequiredForNewUsers: '请为新的 LDAP 用户选择角色。',
            changedFields: '变更字段',
            importSelected: '导入/更新选中',
            importAll: '导入/更新全部',
            importPreview: 'AD/LDAP 导入预览',
            importCompleted: 'AD/LDAP 用户处理完成',
            incompletePreview: 'LDAP 目录未完整读取，请重新从 LDAP 读取。',
            missingUsersWarning: '严格同步将停用 LDAP 中不存在的用户。',
            missingUsersWarningDetail: '{0} 个本地 LDAP 用户将无法继续登录。',
            action: '处理结果',
            reason: '原因',
            noImportChanges: '没有需要处理的变更',
            confirmImport: '确认导入',
            created: '新建',
            updated: '更新',
            reactivated: '恢复',
            ...
            conflicts: '冲突',
            missing: '将停用',
            missingUsersConfirm: 'LDAP 中不存在的 {0} 个用户将被停用并退出登录，是否继续？',
            strictSyncConfirm: '确认严格同步',
            status: { importable: '可导入', update_available: '待更新', up_to_date: '已是最新', conflict: '同名冲突' },
            fields: { name: '用户名', externalDN: '目录 DN', externalID: '外部标识', authSourceStatus: '认证状态' },
            actions: {
                created: '新建',
                updated: '更新',
                reactivated: '恢复',
        ...
        expiresEnterpriseAlert: '温馨提醒：OSS 版许可证将于 [{0}] 天后到期，届时将停止使用所有OSS 版功能。',
        expiresProAlert: '温馨提醒：OSS 版许可证将于 [{0}] 天后到期，届时将停止使用所有OSS 版功能。',
        menu: '高级功能',
        upage: 'AI 建站',
        proAlert: '使用此功能',
        logExport: {
        ...
        waf: {
            ...
            dashHelper: 'OSS 版也可使用全局设置和网站设置中的功能',
        ...
        node: {
            ...
            syncWithMaster: 'OSS 版将默认同步所有数据，可在节点管理中手动调整同步策略。',
            syncProxy: '系统代理设置',
        ...
        user: {
            ...
            auth: {
                ...
                ldap: {
                    ...
                    userSearch: '用户搜索',
                    userSearchDesc: '配置用户搜索基准与过滤器，并映射到 1Panel 用户名字段。',
                    userFilter: '用户过滤器',
                    userFilterTooltip: '按用户名查询时替换 %(user)s，全量读取时替换为 *。',
                    userFilterHelper: '搜索和认证时会对用户名执行 LDAP filter escaping。',
                    attributeMap: '映射属性',
                    attributeMapTooltip: '1Panel 用户名字段（name）到 LDAP 属性的 JSON 映射。',
                    attributeMapHelper: '填写映射到 1Panel 用户名字段（name）的 LDAP 属性。',
                    advanced: '高级设置',
                    advancedDesc: '配置 TLS 和 LDAP 查询分页。',
                    strictSync: '严格同步',
                    strictSyncTooltip: '完整全量处理后，LDAP 中不存在的同源用户将被停用但不会删除。',
                    strictSyncHelper: '仅作用于完整成功的全量处理；导入选中用户时不会停用其他用户。',
                    skipTLSVerify: '跳过 TLS 证书校验',
                    skipTLSVerifyTooltip: '连接 LDAPS 时不校验服务端证书。',
                    skipTLSVerifyHelper: '仅用于可信的自签名证书；该设置与严格同步相互独立。',
                    ...
                    sync: {
                        title: '定时同步',
                        description: '定期读取 LDAP 全量用户，并维护 1Panel 中的 LDAP 用户。',
                        enable: '是否启用',
                        ...
                        strictSyncHelper:
                            '全量查询中缺失的同源 LDAP 用户将被停用并清理会话，但不会删除用户、角色和权限。',
                        safetyTitle: '仅在 LDAP 全量查询完整成功后处理缺失用户',
                        safetyDescription:
                            '查询失败、超时或分页结果不完整时，本次任务不会改变用户状态。同步范围沿用用户搜索中的 Base DN、过滤器和属性映射。',
                        runNow: '立即同步',
                        ...
                        runConfirmDescription: '完整全量查询中缺失的 LDAP 用户将被停用并清理活动会话，是否继续？',
                    ...
                    testLoginDesc: '使用 AD/LDAP 用户凭据执行 bind 验证，仅测试认证能力，不创建 1Panel 登录会话。',
        ...
        customApp: {
            ...
            selectNode: '选择节点',
            licenseHelper: 'OSS 版支持自定义应用仓库功能',
        ...
        alert: {
            ...
            alertConfigProHelper: 'OSS 版额外支持企业微信、钉钉、飞书及短信告警。',
